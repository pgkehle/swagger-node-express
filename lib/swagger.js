...
// TODO-3.0.0 REMOVE
// For backwards compatibility, we just export a new instance of Swagger
...
function Swagger(appHandler) {
  ...
  // PGK: added base path validator support
  this.validatorBase = function (req, basePath, apis, callback) {
    //console.log("this.validatorBase(): basePath: " + basePath);
    callback(true);
  };
  ...
  _.forOwn(this.resources, function (resource, key) {
    ...
    app.get(path, function (req, res) {
      ...
      if (!r) {
        ...
        // PGK: callback version
        self.filterApiListing(req, res, r, function (data) {
...
// PGK: Callback version
Swagger.prototype.filterApiListing = function (req, res, r, callback) {
  var self = this;
  var excludedPaths = [];
//  console.log("Swagger.prototype.filterApiListing(): called");

  if (!r || !r.apis) {
    return callback(self.stopWithError(res, {
      'message': 'internal error',
      'code': 500
    }));
  }

  // PGK: callback version.  Need to ensure each one is handled by using this hash.
  var hash = [];
  _.each(r.apis, function (api, key) {
    hash.push({"key": key, "done": false});
  });

//  console.log("Swagger.prototype.filterApiListing(): hash: " + JSON.stringify(hash));
  ...
  _.each(r.apis, function (api, key) {
    for (var opKey in api.operations) {
      ...
      // PGK: Removing the replacement of the {...} with * which removes more of the path than expected
      var path = api.path.replace(self.formatString, '');
//      var path = api.path.replace(self.formatString, '').replace(/{.*\}/, "*");

      self.canAccessResource(req, path, op.method, function (validated) {
        if (validated === false) {
          excludedPaths.push(op.method + ":" + api.path);
        }

//        console.log("Swagger.prototype.filterApiListing(): hash: " + JSON.stringify(hash));
        var hashEntry = _.find(hash, {'key': key});
//        console.log("Swagger.prototype.filterApiListing(): hashEntry: " + JSON.stringify(hashEntry));
        hashEntry.done = true;
        var notDone = _.find(hash, {'done': false});
//        console.log("Swagger.prototype.filterApiListing(): notDone: " + JSON.stringify(notDone));
...
//          console.log("filterApiListing(): excludedPaths: " + JSON.stringify(excludedPaths));
//          console.log("requiredModels: " + JSON.stringify(requiredModels));
...
// PGK: Turned this into a callback structure
...
Swagger.prototype.canAccessResource = function (req, path, httpMethod, callback) {
  // console.log('canAccessResource(): testing');
  var validated = true;
  ...
  // PGK: callback version.  Need to ensure each one is handled by using this hash.
  var hash = [];
  _.each(self.validators, function (validator, key) {
    hash.push({"key": key, "done": false});
  });

  _.each(self.validators, function (validator, key) {
    validator(req, path, httpMethod, function (result) {
//      console.log('Swagger.prototype.canAccessResource(): validator result: ' + result);
      if (result === false) validated = false;

      var hashEntry = _.find(hash, {'key': key});
      hashEntry.done = true;
//      console.log("Swagger.prototype.canAccessResource(): hashEntry: " + JSON.stringify(hashEntry));
      var notDone = _.find(hash, {'done': false});
//      console.log("Swagger.prototype.canAccessResource(): notDone: " + JSON.stringify(notDone));

      if (!notDone) {
        if (validated === false) {
//          console.log('Swagger.prototype.canAccessResource(): calling callback with validated: ' + validated);
        }
        callback(validated);
      }
    });
...
Swagger.prototype.resourceListing = function (req, res) {
  ...
  // PGK: Hack here to only add to the apis if at least one of the apis are available.
  //  this is done through validatorBase();
  // Need this b/c sometimes these are non-blocking calls
  var hash = [];
  ...
  _.forOwn(self.resources, function (value, key) {
    // Test to see if the key has anything of value
    self.validatorBase(req, key, value.apis, function (valid) {
//      console.log("self.validatorBase(): key: " + key + ", valid: " + valid);
      if (valid == true) {
//        console.log("self.validatorBase(): key: " + key + ", valid: " + valid);
      ...
      var hashEntry = _.find(hash, {'key': key});
      hashEntry.done = true;
//      console.log("self.validatorBase(): hashEntry: " + JSON.stringify(hashEntry));
      var notDone = _.find(hash, {'done': false});
//      console.log("self.validatorBase(): notDone: " + JSON.stringify(notDone));
...
Swagger.prototype.addMethod = function (app, callback, spec) {
  ...
  // PGK: Update to strip /api off the path
  // console.log("path: " + spec.path);
  var pathArr = spec.path.split("/");
  var apiRootPath = pathArr[1];

  if (apiRootPath == "api") {
    pathArr.shift();
//    console.log("apiRootPath: " + apiRootPath);
    apiRootPath = pathArr[1];
  }
...
//  var apiRootPath = spec.path.split(/[\/\(]/)[1];
  ...
  if (allowedMethods.indexOf(currentMethod) === -1) {
    return console.error('unable to add ' + currentMethod.toUpperCase() + ' handler');
  }

  if (_.isArray(callback)) {
    app[currentMethod](fullPath, callback);
  }
  else {
    app[currentMethod](fullPath, function (req, res, next) {
      ...
      // PGK: Removing the replacement of the {...} with * which removes more of the path than expected
      var path = req.url.split('?')[0].replace(self.jsonSuffix, '');
      //var path = req.url.split( '?' )[0].replace( self.jsonSuffix, '' ).replace( /{.*\}/, '*' );

      self.canAccessResource(req, path, req.method, function (validated) {
        if (validated === false) {
...
// PGK: added base path validator support
Swagger.prototype.setBasePathValidator = function (v) {
  this.validatorBase = v;
};
...
// Stop express resource with error code
Swagger.prototype.stopWithError = function (res, error) {
  this.setHeaders(res);
  if (error && error.message && error.code) {
    console.warn("stopWithError(): error: " + JSON.stringify(error));
  }
  else {
    res.send(JSON.stringify({
      'message': 'internal error',
